...
function _drawBuildCanvas(n){
  ...
  if(n<1){_buildLabel(ctx,W,"💫 Complete levels to build the Night Sky!",0,6);return;}
  ctx.fillStyle='#101c28';ctx.fillRect(0,215,W,35);ctx.fillStyle='#182430';ctx.fillRect(0,215,W,5);
  if(n<2){_buildLabel(ctx,W,"🌙 Night sky base — 1/6",1,6);return;}
  // Pulsing star
  ...
  if(n<3){_buildLabel(ctx,W,"💫 Piercing star appears — 2/6",2,6);return;}
  // Water drops
  ...
  if(n<4){_buildLabel(ctx,W,"💧 Water of creation — 3/6",3,6);return;}
  // Quran tablet
  ...
  if(n<5){_buildLabel(ctx,W,"📖 Decisive Quran — 4/6",4,6);return;}
  ...
  if(n<6){_buildLabel(ctx,W,"🌧️ Rain and earth — 5/6",5,6);return;}
